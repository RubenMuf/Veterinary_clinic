...
class PetAdmin(admin.ModelAdmin): # класс чтобы в админе отображались параметры питомцев
    fields = ['nick_name', 'photo', 'post_photo', 'view', 'breed', 'slugPet', 'age', 'weight', 'gender', 'onwer', 'medical_history']  # отображение полей формы, для редактирование записей объектов
    readonly_fields = ['post_photo']  # чтобы фотография выводилась в панеле редактирования
    list_display = ('id', 'nick_name', 'post_photo', 'view', 'breed', 'age', 'weight', 'gender', 'weight', 'brief_info', 'time_create') # отображение параметров из таблицы на титульном листе, какие есть объекты
    ...
    def brief_info(self, pet: Pet): # метод для вывода в админ для контента данного питомца
        if pet.medical_history:
            return f'{len(pet.medical_history)} символов.' #
        else:
            return 'Нет описания'
...
class VeterinarianAdmin(admin.ModelAdmin): # класс чтобы в админе отображались параметры питомцев
    fields = ['specialist', 'photo', 'post_photo', 'firstname', 'lastname', 'email', 'tel', 'history', 'certificates']
    readonly_fields = ['post_photo']  # чтобы фотография выводилась в панеле редактирования
    list_display = ('id', 'post_photo', 'specialist', 'firstname', 'lastname', 'email', 'brief_info', 'tel', 'certificates') # отображение параметров из таблицы
    ...
    @admin.display(description='О докторе', ordering='id') # декоратор для названия этого метода, 2й пар для сортировки, можно указывать только который есть в таблице
    def brief_info(self, vet: Veterinarian): # метод для вывода в админ для контента данной женщины
        if vet.history:
            return f'{len(vet.history)} символов.'
        else:
            return 'Нет описания'

    @admin.display(description='Фотография', ordering='specialist')
